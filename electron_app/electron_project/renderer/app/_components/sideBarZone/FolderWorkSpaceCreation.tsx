    ...
    const { data, isFetching, isError, refetch } = useQuery<any>({
        ...
        queryFn: async () => {
            return new Promise((resolve, reject) => {
                const handleSetFolderPathString = (event, args) => {
                    resolve(args);
                    // Cleanup: remove the listener after resolving
                    window.electron.stopReceiveSelectedFolderPath(handleSetFolderPathString);
                };
                
                window.electron.receiveSelectedFolderPath(handleSetFolderPathString);
                window.electron.selectFolderPath();
